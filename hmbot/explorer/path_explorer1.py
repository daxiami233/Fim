...
weights = {
  "insert_remove_bonus": 0.8023402950518024,
  "type": 2.7395396261532095,
  "id": 1.9063129594672958,
  "clickable": 1.3222616156660008,
  "text": 1.4558979411861326,
  "bounds": 0.1217482229340051,
  "enabled": 0.46768383197386343
}

def get_cost_functions(weights):
    """根据权重字典生成一套完整的代价函数。"""
    def insert_cost(node):
        if (node.attribute.get('clickable') == 'true' or
            node.attribute.get('id') or
            node.attribute.get('text')):
            return weights['insert_remove_bonus']
        return 1.0
    ...
    def update_cost(a, b):
        cost = 0.0
        if a.attribute.get('type') != b.attribute.get('type'): cost += weights['type']
        id_a, id_b = a.attribute.get('id', ''), b.attribute.get('id', '')
        if id_a and id_b and id_a != id_b: cost += weights['id']
        if a.attribute.get('clickable') != b.attribute.get('clickable'): cost += weights['clickable']
        if a.attribute.get('text', '') != b.attribute.get('text', ''): cost += weights['text']
        if a.attribute.get('bounds') != b.attribute.get('bounds'): cost += weights['bounds']
        if a.attribute.get('enabled') != b.attribute.get('enabled'): cost += weights['enabled']
        return cost
        
    return insert_cost, remove_cost, update_cost

insert_cost_func, remove_cost_func, update_cost_func = get_cost_functions(weights)
...
## 决策原则 (请严格遵守!)
1.  **理解功能路径**: “已总结的成功策略”描述的是**功能性**路径，而非具体控件的路径。例如，策略“我是通过点击联系人列表中的一个用户头像进入其主页的”意味着“点击任意用户头像”这条**功能路径**已经被探索过了。
    *   你的首要任务是找到一条在**功能上**与“已总结的成功策略”和“已探索动作”**完全不同**的新路径来达成“{target}”。

2.  **杜绝功能重复**:
    *   **识别功能等价操作**: 在选择“Available Widgets”中的控件时，要先判断这个操作在功能上是否与已成功的策略重复。如果一个策略是“点击列表项进入详情页”，那么在当前页面点击任何一个相似的列表项都属于**功能重复**，应当避免。
...
'''
    ...
    def is_page_exist(self, page: Page):
        ...
        """
        for page_node in self.pages:
            if self._is_pages_same(page_node.page, page):
    ...
    def _is_pages_same(self, page1, page2):
        ...
        """
        if page1.info.ability != page2.info.ability:
            return False
        else:
            ...
            distance = zss.distance(
                page1.vht._root, page2.vht._root,
                get_children=VHTNode.get_children,
                insert_cost=insert_cost_func, 
                remove_cost=remove_cost_func, 
                update_cost=update_cost_func
            )
            count1, count2 = page1.vht.get_node_count(), page2.vht.get_node_count()
